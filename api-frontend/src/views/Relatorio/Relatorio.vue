<template>
  <div class="row relatorio">
    <div class="col-3">
      <p class="bold">p</p>
      <select class="form-select" aria-label=".form-select-lg example"></select>
    </div>
    <div class="col-1" style="margin-top: 40px">
      <button @click="gerar()" type="button" class="btn btn-outline-primary">
        Gerar relatório
      </button>
    </div>
  </div>
...
export default class Relatorio extends Vue {
  async gerar() {
    try {
      ...
      link.download = "extrato_hora.xls";
      link.click();
    } catch (error) {
