<template>
    ...
    <div class="row">
        <div class="col-4">
            ...
            <select class="form-control" v-model="filter.idCr">
                <option v-for="item in crs" :key="item.idCr" :value="item.idCr">{{ item.nomeCr }}</option>
            </select>
        </div>
        <div class="col-4">
            <label>Cliente</label>
            <select class="form-control" v-model="filter.idCliente">
                <option v-for="item in clients" :key="item.idCliente" :value="item.idCliente">{{ item.razaoSocialCliente }}</option>
            </select>
        </div>
        <div class="col-4">
            <label >Inicio</label>
    ...
    <div class="row mt-2">
        ...
        <div class="col-5 mt-4">
            <button type="button" v-on:click="getData()" class="btn btn-outline-primary">
                Carregar Dashboard
            </button>
        </div>
    </div>
...
export default class DashboardView extends Vue {
    ...
    getData(){
        ...
        http.get('/dashboard', {params:request})
            .then(response => {
                const user = useAuth().getUser();
                const filteredData = response.data.filter((x:any) => x.idUsuario == user.id);
